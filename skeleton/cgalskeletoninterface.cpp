...
skelComponents CgalSkeletonInterface::computeSkeleton(const char* path)
{
...
//        qDebug() << "Is quad? " << (face->is_quad()) << "\n";
    ...
    edgesTAndCoords.listaEdge = listaLati;
    edgesTAndCoords.listaTriangoli = triangles;
    edgesTAndCoords.listaPunti = coord;

    return edgesTAndCoords;

}

//std::list<std::pair<Pointd, Pointd>> CgalSkeletonInterface::computeMesoSkeleton(const char* path)
//{
//    std::ifstream input(path);
//    std::pair<Pointd, Pointd> lato;
//    std::list<std::pair<Pointd, Pointd>> listaLati;
