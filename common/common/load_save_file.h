...
namespace Common {
    ...
    bool loadMeshFromObj(const std::string &filename, std::list<T>& coords, std::list<V>& triangles, MeshType &meshType, int &modality = Dummies::dummyInt, std::list<C> &verticesNormals = Dummies::dummyListDouble, std::list<Color> &verticesColors = Dummies::dummyListColor, std::list<Color> &triangleColors = Dummies::dummyListColor, std::list<W> &faceSizes = Dummies::dummyListUnsignedInt);
